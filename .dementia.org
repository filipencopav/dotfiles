...
* Sync to backups
First, open drive with =cryptsetup open=. Mount to =/path/to/drive=
Second, =export BACKUPDIR=/path/to/drive=
Third:
#+begin_src bash
  rsync --info=skip0 --exclude-from="$XDG_CONFIG_HOME/rsync-homedir-local.txt" -aP build devel dox dskt dwn mus pix proj tmpl vids "$BACKUPDIR"
#+end_src
