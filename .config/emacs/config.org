...
#+begin_src emacs-lisp
  (defvar p/*curated-elpaca-package-list*
    ()
    "Alist of the shape ((ID . PLIST) ...) for elpaca")

  (defun p/custom-elpaca-menu (request &optional item)
    (when (eq request 'index)
      (if item
          (alist-get item p/*curated-elpaca-package-list*)
        p/*curated-elpaca-package-list*)))

  (defun p/add-recipe (recipe)
    (let* ((id (car recipe))
           (recipe (cdr recipe))
           (source "PAVEL'S CUSTOM MENU")
           (val `(:source ,source :recipe ,recipe)))
      (setf (alist-get id p/*curated-elpaca-package-list*)
            val)))
#+end_src
...
* Look & feel
** Theme
I use modus themes, particularly modus-operandi. They are nice because they are made for accessibility and minimal eye strain in mind.

#+begin_src emacs-lisp
  (add-hook 'after-init-hook (lambda () (load-theme 'modus-operandi t)))
#+end_src
...
#+begin_src emacs-lisp
  (leaf org-roam
    ...
    (setq org-roam-directory (thread-first
                               "org/roam/"
                               (expand-file-name (getenv "XDG_DOCUMENTS_DIR"))
                               (file-truename)))
...
#+begin_src emacs-lisp
  (leaf rust-mode
    :after (eglot)
    :pre-setq (rust-mode-treesitter-derive . t)
    :config
    (add-to-list
...
*** Nix
#+begin_src emacs-lisp
  ;; (p/add-recipe '(nix-ts-mode :fetcher github
  ;;                             :repo "nix-community/nix-ts-mode"
  ;;                             :ref "319831712125e8c9e4f14b66def518d7a633685c"
  ;;                             :package "nix-ts-mode"
  ;;                             :source "github"
...
#+begin_src emacs-lisp
  (leaf astro-ts-mode
    :after (eglot typescript-ts-mode)
    :config
